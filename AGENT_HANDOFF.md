...
- Дизайн основан на согласованной странице приложений: чёрный фон, бирюзовые волны, контурная типографика и планета с официальным знаком. Не возвращать старые split-hero с декоративными панелями и чужими логотипами.
- Старый проект `app/` удалён из исходников. Его маршруты, шаблоны, SEO, сервис-воркер, изображения и манифест больше не входят в репозиторий. Docker использует отдельные каталоги `wavebreak-*`.
- Стили публичного сайта собраны в одном `wavebreak-site.css`, без накопленных вариантов старых редизайнов. Скрипт учитывает reduced motion и останавливает волну вне экрана.
- Тарифы показывают валюту и лимиты из API. При недоступности API показывается сообщение, а не демонстрационные цены.
- Клиентский кабинет остаётся только в приложениях. Кнопки загрузки пока заглушки; не подставлять фиктивные ссылки на сборки.
- Не изменены API, админка, рабочие транспорты и подключения пользователей.
- Шрифты раздаются с собственного домена (`public/fonts`, лицензии OFL приложены). Внешней зависимости от Google Fonts больше нет.
- Проверки: `cd wavebreak-web && php artisan test`. Браузерная проверка: `node scripts/validate-public-site.mjs` с переменными `PLAYWRIGHT_MODULE` и при необходимости `SITE_URL`.
- Проверка 18.09.2026: Laravel-тесты публичных страниц; 20 браузерных сценариев (4 страницы, ширины 320/390/768/1440/1920), reduced motion, отображение без JS и корректность JSON-LD. Проверяются меню, FAQ, изображения, пиксели canvas, горизонтальный overflow и отсутствие обрезанного текста в превью. После первого деплоя те же 20 сценариев прошли на HTTPS-домене с настоящими тарифами; старый бренд не найден в HTML всех четырёх страниц.
- Локальные данные старого проекта (непубликуемые .env, SQLite и вложенная резервная Git-копия) сохранены в игнорируемом `.artifacts/legacy-private/`. Исходники, старые собранные ассеты, зависимости и кэши из корневого `app/` удалены. Архив не добавлять в Git и не выкладывать.

Деплой только сайта, из `wavebreak-infrastructure`:

```bash
docker compose --env-file .env.pilot -p wavebreak_pilot -f docker-compose.pilot.yml build wavebreak-web
docker compose --env-file .env.pilot -p wavebreak_pilot -f docker-compose.pilot.yml up -d --no-deps wavebreak-web
```

Не перезапускать ноду, transport-контейнеры и базу ради обновления сайта.
